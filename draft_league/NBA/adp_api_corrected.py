...
def get_adp_rankings():
    ...
    query = """
        ...
        LEFT JOIN (
            ...
            WHERE season = 2026
        ) ndr ON a.player_id = ndr.player_id
        WHERE a.season = 2026
    """
    ...
    # 位置筛选
    if position:
        query += " AND ndr.position = ?"
        params.append(position)
    
    # 搜索功能
    if search:
        query += " AND ndr.player_name LIKE ?"
        params.append(f"%{search}%")
    
    query += " ORDER BY a.adp LIMIT ?"
    params.append(limit)
    ...
    for player in players:
        result.append({
            ...
            'player_name': player['player_name'] or player['player_id'],
...
def get_adp_player_detail(player_id):
    ...
    cursor.execute("""
        ...
        WHERE a.player_id = ? AND a.season = 2026
    """, (player_id,))
...
def get_adp_stats():
    ...
    # 按位置统计（从nba_draft_results）
    cursor.execute("""
        SELECT ndr.position, COUNT(DISTINCT a.player_id) as count
        FROM adp_rankings a
        JOIN nba_draft_results ndr ON a.player_id = ndr.player_id
        WHERE a.season = 2026 AND ndr.season = 2026 AND ndr.position IS NOT NULL
        GROUP BY ndr.position
        ORDER BY ndr.position
    """)
