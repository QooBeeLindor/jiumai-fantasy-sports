"""
完整API - ADP + 战绩 + FA排行榜 + 比赛日历
（修复版：优雅处理缺失的transactions表）
"""
...
def get_fa_rankings():
    ...
    # 检查表是否存在
    if not check_table_exists('fa_transactions'):
        return jsonify({
            'success': True,
            'data': [],
            'total': 0,
            'message': 'fa_transactions表不存在，无交易数据',
            'filters': {
    ...
    # 注意：列名是 transaction_type，值是大写的 'ADD' 和 'DROP'
    query = f"""
        SELECT 
            nba_player_key as yahoo_player_id,
            nba_player_name,
            '' as nba_position,
            '' as nba_team,
            ...
            SUM(CASE WHEN transaction_type = 'ADD' THEN 1 WHEN transaction_type = 'DROP' THEN -1 ELSE 0 END) as net_adds
        FROM fa_transactions
        WHERE season = 2026 {date_filter}
        GROUP BY nba_player_key, nba_player_name
    ...
    for idx, player in enumerate(fa_players, 1):
        ...
        result.append({
            ...
            'player_id': player['yahoo_player_id'],
            'player_name': player['nba_player_name'],
            'position': player['nba_position'] if player['nba_position'] else 'N/A',
            'team': player['nba_team'] if player['nba_team'] else 'FA',
...
def get_fa_player_detail(player_id):
    ...
    if not check_table_exists('fa_transactions'):
        return jsonify({
            'success': False,
            'error': 'fa_transactions表不存在，无交易数据'
        }), 404
    ...
    cursor.execute("""
        SELECT DISTINCT
            ...
            nba_player_name
        FROM fa_transactions
        WHERE nba_player_key = ? AND season = 2026
        LIMIT 1
    """, (player_id,))
    ...
    cursor.execute("""
        SELECT 
            SUM(CASE WHEN transaction_type = 'ADD' THEN 1 ELSE 0 END) as total_adds,
            SUM(CASE WHEN transaction_type = 'DROP' THEN 1 ELSE 0 END) as total_drops,
    ...
    cursor.execute("""
        ...
        WHERE t.nba_player_key = ? AND t.season = 2026
    ...
    # 尝试从transaction_date提取周次
    cursor.execute("""
        SELECT 
            CAST(strftime('%W', transaction_date) AS INTEGER) as week,
    ...
    result = {
        'player_id': player['nba_player_key'],
        'player_name': player['nba_player_name'],
        'position': 'N/A',
        'team': 'FA',
        'stats': {
...
def get_fa_stats():
    ...
    if not check_table_exists('fa_transactions'):
        return jsonify({
            ...
            'data': {
                ...
                'total_fa_players': 0,
                'hottest_player': None,
                'message': '数据库中没有fa_transactions表，无交易数据'
            }
        })
    ...
    cursor.execute("""
        SELECT COUNT(*) as total
        FROM fa_transactions
        WHERE transaction_type = 'ADD' AND season = 2026
    """)
    ...
    cursor.execute("""
        SELECT COUNT(*) as total
        FROM fa_transactions
        WHERE transaction_type = 'DROP' AND season = 2026
    """)
    total_drops = cursor.fetchone()['total']
    
    cursor.execute("""
        SELECT COUNT(DISTINCT nba_player_key) as total
        FROM fa_transactions
        WHERE season = 2026
    """)
...
def health_check():
    ...
    # 检查各表是否存在
    tables_status = {
        'adp_rankings': check_table_exists('adp_rankings'),
        'league_stats': check_table_exists('league_stats'),
        'matches': check_table_exists('matches'),
        'fa_transactions': check_table_exists('fa_transactions')
    }
...
# ============================================================================
# League Detail API (分盟详细页面)
# ============================================================================

@app.route('/api/league/<league_id>/detail', methods=['GET'])
def get_league_detail(league_id):
    ...
    try:
        ...
        cursor.execute("""
            SELECT DISTINCT league_id, league_name
            FROM overall_roto_rankings
            WHERE season = 2026 AND league_id = ?
            LIMIT 1
        """, (league_id,))
        ...
        cursor.execute("""
            ...
            FROM overall_roto_rankings
            WHERE season = 2026 AND league_id = ?
            ORDER BY total_roto_points DESC
        """, (league_id,))
        ...
        teams_data = []
        for team in teams:
            stats = json.loads(team['stats_json']) if team['stats_json'] else {}
            team_key = team['team_key']
            
            # 获取standings信息
            standings_info = standings_map.get(team_key, {})
            ...
            teams_data.append({
                ...
                'total_roto_points': round(team['total_roto_points'], 2),
                'stats': stats,
                'league_rank': standings_info.get('rank', None),
                'wins': standings_info.get('wins', 0),
                'losses': standings_info.get('losses', 0),
                'ties': standings_info.get('ties', 0),
                'percentage': standings_info.get('percentage', 0),
                'games_back': standings_info.get('games_back', '-')
            })
        
        # 5. 计算联赛统计
        total_teams = len(teams_data)
        avg_roto_points = sum(t['total_roto_points'] for t in teams_data) / total_teams if total_teams > 0 else 0
        max_roto_points = max((t['total_roto_points'] for t in teams_data), default=0)
        min_roto_points = min((t['total_roto_points'] for t in teams_data), default=0)
        ...
        return jsonify({
            ...
            'data': {
                ...
                'stats': {
                    ...
                    'avg_roto_points': round(avg_roto_points, 2),
                    'max_roto_points': round(max_roto_points, 2),
                    'min_roto_points': round(min_roto_points, 2)
                }
            }
        })
...
if __name__ == '__main__':
    ...
    print()
    print("ADP API:")
    print("  * GET /api/adp/rankings      - ADP Rankings")
    print("  * GET /api/adp/stats         - ADP Stats")
    print()
    print("Standings API:")
    print("  * GET /api/standings/rankings        - Standings")
    print("  * GET /api/standings/player/<id>     - Player Details")
    print("  * GET /api/standings/leagues         - League List")
    print("  * GET /api/standings/stats           - Stats")
    print()
    print("FA Rankings API:")
    print("  * GET /api/fa/rankings               - FA Rankings")
    print("  * GET /api/fa/player/<id>            - FA Player Details")
    print("  * GET /api/fa/stats                  - FA Stats")
    print()
    print("Schedule API:")
    print("  * GET /api/schedule/weeks            - Week List")
    print("  * GET /api/schedule/week/<week>      - Week Schedule")
    print("  * GET /api/schedule/player/<id>      - Player Schedule")
    print("  * GET /api/schedule/stats            - Schedule Stats")
    print()
    print("Overall Roto API:")
    print("  * GET /api/overall_roto/rankings     - Overall Roto Rankings")
    print("  * GET /api/overall_roto/team/<key>   - Team Details")
    print("  * GET /api/overall_roto/leagues      - League List")
    print("  * GET /api/overall_roto/stats        - Stats")
    print()
    print("League Detail API:")
    print("  * GET /api/league_standings          - All League Standings")
    print("  * GET /api/league/<id>/detail        - League Detail Page")
    print()
    ...
    if check_table_exists('fa_transactions'):
        print("  [OK] fa_transactions table exists")
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) as total FROM fa_transactions WHERE season = 2026")
        count = cursor.fetchone()['total']
        conn.close()
        print(f"      Found {count} transactions")
    else:
        print("  [WARN] fa_transactions table not found")
    
    if check_table_exists('overall_roto_rankings'):
        print("  [OK] overall_roto_rankings table exists")
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) as total FROM overall_roto_rankings WHERE season = 2026")
        count = cursor.fetchone()['total']
        conn.close()
        print(f"      Found {count} teams")
    else:
        print("  [WARN] overall_roto_rankings table not found")
        print("      Please run: python calculate_overall_roto.py")
