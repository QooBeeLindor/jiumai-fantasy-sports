...
def get_fa_rankings():
    """获取FA排行榜（如果没有transactions表则返回空数据）"""
    ...
    if not check_table_exists('transactions'):
        return jsonify({
            ...
            'data': [],
            'total': 0,
            'message': 'transactions表不存在，无交易数据',
            'filters': {
...
def get_fa_player_detail(player_id):
    """获取FA球员详情（如果没有transactions表则返回空数据）"""
    
    if not check_table_exists('transactions'):
        return jsonify({
            'success': False,
            'error': 'transactions表不存在，无交易数据'
        }), 404
...
def get_fa_stats():
    """获取FA统计概览（如果没有transactions表则返回提示）"""
    ...
    if not check_table_exists('transactions'):
        return jsonify({
            ...
            'data': {
                ...
                'total_fa_players': 0,
                'hottest_player': None,
                'message': '数据库中没有transactions表，无交易数据'
            }
        })
...
if __name__ == '__main__':
    ...
    if check_table_exists('transactions'):
        print("  ✅ transactions表存在 - FA功能可用")
    else:
        print("  ⚠️  transactions表不存在 - FA功能将返回空数据")
        print("     建议：运行 check_all_tables.py 查看所有表")
