...
def roster(league_id, team_key):
    ...
    try:
        ...
        for p in roster_data:
            ...
            # 标准化状态值（Yahoo API可能返回不同格式）
            if isinstance(raw_status, str):
                status_lower = raw_status.lower()
                if status_lower in ['', '-', 'healthy']:
                    status = 'active'
                elif 'il' in status_lower or 'inj' in status_lower:
                    status = 'IL'
                elif status_lower in ['o', 'out']:
                    status = 'OUT'
                elif 'dtd' in status_lower or 'day' in status_lower:
                    status = 'DTD'
