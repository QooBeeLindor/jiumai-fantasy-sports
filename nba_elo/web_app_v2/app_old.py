...
def index():
    ...
    try:
        ...
        players_raw = conn.execute('''
            SELECT player_id, nickname, current_elo, initial_elo,
        ...
        for idx, player in enumerate(players_raw, 1):
            ...
            ''', (player['player_id'], player['player_id'], 
                  player['player_id'], player['player_id'],
                  player['player_id'], player['player_id'])).fetchone()
...
@app.route('/leagues')
def leagues():
    """联赛对比页面 - 从matches表构建联赛数据"""
    try:
        conn = get_db_connection()
        
        # 从matches表获取所有联赛（因为leagues表是空的）
        ...
        for league in leagues_raw:
            ...
            stats = conn.execute('''
                SELECT 
                    COUNT(DISTINCT team1_manager_id) + COUNT(DISTINCT team2_manager_id) as total_managers,
            ...
            players_raw = conn.execute('''
                ...
                WHERE p.player_id IN (
                    SELECT DISTINCT team1_manager_id FROM matches WHERE league_id = ?
                    UNION
                    SELECT DISTINCT team2_manager_id FROM matches WHERE league_id = ?
                )
            ...
            if players_raw:
                ...
                avg_score_result = conn.execute('''
                    SELECT AVG((team1_score + team2_score) / 2.0) as avg_score
                    FROM matches
                    WHERE league_id = ?
                ''', (league_id,)).fetchone()
                avg_score = avg_score_result['avg_score'] if avg_score_result and avg_score_result['avg_score'] else 0
            else:
                avg_elo = 0
                avg_score = 0
        ...
        for player in players_raw:
            ...
            for week in weeks:
                ...
                elo_result = conn.execute('''
                    ...
                    LIMIT 1
                ''', (player_id, player_id, player_id, player_id, week)).fetchone()
